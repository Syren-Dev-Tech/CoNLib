...
public class CustomCreature extends PathfinderMob implements GeoEntity { // NOSONAR - Ignore parent class limit
    ...
    private <E extends GeoEntity> PlayState animationPredicate(AnimationState<E> event) {
        ...
        if (currentAnimation != null) {
            Scylla.LOGGER.info("Current animation: {}", currentAnimation.animation().name());
        } else {
            Scylla.LOGGER.info("No current animation.");
        }
        ...
        Scylla.LOGGER.info("Setting animation to default animation: {}", DEFAULT_ANIMATION);
