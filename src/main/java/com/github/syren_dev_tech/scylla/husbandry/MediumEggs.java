...
public class MediumEggs {
    ...
    public static final <T extends Animal> Supplier<MediumEgg<T>> create(ModRegister register, String name,
            MediumEggData<T> eggData) {
        return create(register, name, Properties.copy(Blocks.SNIFFER_EGG), eggData);
    }

    public static final <T extends Animal> Supplier<MediumEgg<T>> create(ModRegister register, String name,
            MediumEggData<T> eggData, ResourceKey<CreativeModeTab> creativeTab) {
        return create(register, name, Properties.copy(Blocks.SNIFFER_EGG), eggData, creativeTab);
    }

    public static final <T extends Animal> Supplier<MediumEgg<T>> create(ModRegister register, String name,
            Properties properties, MediumEggData<T> eggData) {
        return register.blockRegistry.register(name, () -> new MediumEgg<T>(properties, eggData));
    }

    public static final <T extends Animal> Supplier<MediumEgg<T>> create(ModRegister register, String name,
            Properties properties, MediumEggData<T> eggData, ResourceKey<CreativeModeTab> creativeTab) {
